...
public class SmoothieSlider : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (beginSlider == true)
        {
            ...
            if (timer >= slider.maxValue)
            {
                ...
                slider.value = 0;
                beginSlider = false;
                GameObject.Find("Smoothie Gen").GetComponent<FoodGen>().Smoothie = true;
            }
        }
    }
    ...
    public void BeginSlider()
    {
        if(bScript !=null)
        {
            if (GameObject.Find("Smoothie Gen").GetComponent<FoodGen>().Smoothie == false && bScript.bOut == false && fScript.fireOut == false)
            {
                beginSlider = true;
        ...
        {
            if (GameObject.Find("Smoothie Gen").GetComponent<FoodGen>().Smoothie == false && fScript.fireOut == false)
            {
                beginSlider = true;
