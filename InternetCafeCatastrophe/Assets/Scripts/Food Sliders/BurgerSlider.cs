...
public class BurgerSlider : MonoBehaviour {
    ...
    void Update()
    {
        ...
        if (beginSlider == true)
        {
            ...
            if (timer >= slider.maxValue)
            {
                ...
                slider.value = 0;
                beginSlider = false;
                GameObject.Find("Burger Gen").GetComponent<FoodGen>().Burger = true;
            }
        }
    }
    ...
    public void BeginSlider()
    {
        if(bScript != null)
        {
            if (GameObject.Find("Burger Gen").GetComponent<FoodGen>().Burger == false && bScript.bOut == false && fScript.fireOut == false)
            {
                beginSlider = true;
        ...
        {
            if (GameObject.Find("Burger Gen").GetComponent<FoodGen>().Burger == false && fScript.fireOut == false)
            {
                beginSlider = true;
